...
<div class="mypage-wrapper in">
    ...
    <div class="mypage-content">
        ...
        <table class="reservation-search-result-table" border="1" cellpadding="8" cellspacing="0"
            ...
            <tr>
                <td colspan="7" class="empty">예약번호를 입력하세요.</td>
            </tr>
            <% } else if (foundReservation == null) { %>
            <tr>
                <td colspan="7" class="empty">예약내역을 찾을 수 없습니다.</td>
            </tr>
            <% } else { %>
            <tr>
                <td><%= foundReservation.getId() %>
                </td>
                <td><%= foundReservation.getOrderDate() %>
                </td>
                <td><%= foundReservation.getName() %>
                </td>
                <td><%= foundReservation.getDepartDate() %>
                </td>
                <td><%= foundReservation.getPeople() %>명</td>
                <td><%= String.format("%,d", foundReservation.getTotalPrice()) %>원</td>
                <td><%= foundReservation.getStatus() %>
                </td>
            </tr>
            <% } %>
            </tbody>
